...
class ImportExcelHelper {
    ...
    foreach ($data as $index => $row) {
        try {
            // Appelle la fonction métier spécifique (ex: checkAndCreateBienByExcel)
            $callback($row, $projet_id);
        } catch (\Exception $e) {
            $hasError = true;
            $import->statut = 2; // bloqué
            $import->message_echou = "Erreur ligne " . ($index + 1) . " : " . $e->getMessage();
            $import->ligne_echou = $index + 1;
            $import->date_echou = now();
            $import->save();

            // Si tu veux arrêter à la première erreur, décommente la ligne suivante :
            // break;
