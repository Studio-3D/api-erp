...
return new class extends Migration
{
    ...
    {
        Schema::table('facebook_configurations', function (Blueprint $table) {
            if (!Schema::hasColumn('facebook_configurations', 'webhook_verify_token')) {
                $table->string('webhook_verify_token')->nullable()->after('projet_id');
            }
            if (!Schema::hasColumn('facebook_configurations', 'webhook_enabled')) {
                $table->boolean('webhook_enabled')->default(false)->after('webhook_verify_token');
            }
            if (!Schema::hasColumn('facebook_configurations', 'webhook_subscriptions')) {
